class Quality::SpcController < ApplicationController
  def index
    @item_names = InspectionItem.for_tenant(Current.tenant).distinct.pluck(:item_name)
    ...
    service = SpcCalculatorService.new(tenant: Current.tenant, item_name: @item_name, from: @from, to: @to)
